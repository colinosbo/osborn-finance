...
export default function Ledger({ toast }: { toast: Toast }) {
  ...
  useEffect(() => { api<{ total: number }>('/api/transactions?limit=1').then(r => setHasData(r.total > 0)).catch(() => setHasData(false)); }, []);
    ...
    <>
      ...
      {hasData === false ? (
        <EmptyState
          ...
          sub="Once you connect a bank, every transaction lands here — automatically categorized and groupable by merchant, so repeats like Starbucks total up together."
        />
      ) : (
      <div className="panel">
